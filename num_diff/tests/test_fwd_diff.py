...
Test the calculation of a functions (partial) derivative using a forward finite
difference scheme of order 2 or 3.
...
from forward_diff import fwd_diff_first
...
# Test cases
	...
	for i in range(1,20):
		x = i*0.01
		der_num = fwd_diff_first(x, var = 0, func = f, delta_x = 0.0001, order = m)
	...
	for i in range(1,20):
		...
		y = i*1.2
		r = [x,y]
		der_num = fwd_diff_first(*r, var = 0, func = f, delta_x = 0.001, order = m)
	...
	for i in range(1,20):
		...
		y = i*1.2
		r = [x,y]
		der_num = fwd_diff_first(*r, var = 1, func = f, delta_x = 0.001, order = m)
